...
SELECT *, SUM(cp.quantity * cp.cost_to_customer_per_qty) as total
from customer  c
...
SELECT customer_id, STRFTIME('%m', market_date) as month, STRFTIME('%Y', market_date) as year, SUM(quantity * cost_to_customer_per_qty) as total
FROM customer_purchases
